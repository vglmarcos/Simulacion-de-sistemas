...
simulacionP <- function(x){
  ...
  clusterExport(cluster, "simulacionP")
  ...
  clusterExport(cluster, "unelo")
  clusterExport(cluster, "roto")
  ...
  for (replicas in 1:30) {
    ...
    for (paso in 1:duracion) {
      assert(sum(cumulos) == n)
      cumulos <- integer()
      cumulos <- as.vector(parSapply(cluster, 1:(dim(freq)[1]), roto))
      acomoda <- c()
      ...
      assert(sum(freq$num * freq$tam) == n)
      cumulos <- integer()
      cumulos <- as.vector(parSapply(cluster, 1:(dim(freq)[1]), unelo))
      acomoda <- c()
      for(i in 1:length(cumulos)){
        acomoda <-c(acomoda, cumulos[[i]])
      }
...
time <- function(x){
  return(as.numeric(system.time(min(simulacion()))[3]))
}

timeP <- function(x){
  return(as.numeric(system.time(min(simulacionP()))[3]))
}

replicas <- 1

resultados <- data.frame(replica = integer(), tiempo = double(), tipo = character())

tipo <- "Secuencial"

for(i in 1:replicas){
  tiempo <- time()
  resultados <- rbind(resultados, data.frame(replica = i, tiempo = tiempo, tipo = tipo))
}
